...
function SmallSpark({ labels, values }){
  ...
  useEffect(()=>{
    ...
    const chart = new Chart(ctx, {
      type: 'line',
      data: { labels, datasets: [{ data: values, tension:.35, borderWidth:2, pointRadius:0, fill:false }] },
      options: {
  ...
  return (
    <div className="kpi card glow rounded-2xl p-4 fade-in">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`badge px-2 py-1 rounded-lg text-xs ${badge}`}>{better ? '▲ vs Budget' : '▼ vs Budget'}</span>
      </div>
