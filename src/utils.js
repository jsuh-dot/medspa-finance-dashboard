export const currencyMetrics = new Set(['Beauty Rev','Infusion Rev','Membership Rev','Store Rev','Tech Rev','Wellness Rev','Total Rev','Revenue','COGS','Sales & Marketing','R&D','G&A','OpEx','EBITDA','CAC','LTV']);
...
// Return best-available "Revenue" scalar from row
function computeRevenueFromRow(row){
  const rev = parseNumber(row['Revenue']);
  if (rev!=null) return rev;
  const total = parseNumber(row['Total Rev']);
  if (total!=null) return total;
  const cats = ['Beauty Rev','Infusion Rev','Membership Rev','Store Rev','Tech Rev','Wellness Rev'];
...
export function deriveMetrics(row){
  ...
  let Revenue = parseNumber(out['Revenue']);
  let TotalRev = parseNumber(out['Total Rev']);
  if (Revenue==null){
    Revenue = computeRevenueFromRow(out);
    if (Revenue!=null) out['Revenue'] = Revenue;
  }
  if (TotalRev==null && Revenue!=null){
    out['Total Rev'] = Revenue;
    TotalRev = Revenue;
  }
  if (Revenue==null && TotalRev!=null){
    out['Revenue'] = TotalRev;
    Revenue = TotalRev;
  }
